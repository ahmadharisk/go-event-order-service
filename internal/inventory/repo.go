...
func (r *Repo) Release(ctx context.Context, orderID string) error {
	...
	defer rows.Close()
	var items []struct{ pid string; qty int }
	for rows.Next() {
		var pid string
		var qty int
		...
		items = append(items, struct{ pid string; qty int }{pid, qty})
	}
	if err := rows.Err(); err != nil {
		return err
	}
	for _, it := range items {
		if _, err := tx.Exec(ctx, `UPDATE inventory SET stock=stock+$1, version=version+1 WHERE product_id=$2`, it.qty, it.pid); err != nil {
			return err
		}
	}
	return tx.Commit(ctx)
